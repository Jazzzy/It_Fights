...
enum Character { MYSELF, OTHER };

enum CharacterAction { MOVING, STANDING, ATTACKING, PARRYING, COOLING_DOWN };
...
class AIObserver {
 public:
  ...
  void calculateFitness(FightState_Discrete* discreteState,
                        FightState continuousState);

  MyCharacterState_Discrete discretizeMyCharacter(FightState continuousState);
  OtherCharacterState_Discrete discretizeOtherCharacter(
