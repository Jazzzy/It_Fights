...
Level_00_GO_EnemyCharacter::Level_00_GO_EnemyCharacter(Scene* scene,
                                                       sf::Vector2f position,
                                                       AIObserver* observer,
                                                       Position playerPosition,
                                                       EnemyType type)
    : Level_00_GO_BasicCharacter(scene, position, playerPosition) {
  if (playerPosition == PLAYER_1) {
    this->hurtbox = new BoxCollider(
        sf::Vector2f(hurtboxSize_x, hurtboxSize_y),

        // Function that returs the position in which the location collider
        // should be positioned
        [this]() -> sf::Vector2f {
          return sf::Vector2f(this->position.x - hurtboxOffset_x,
                              this->position.y - hurtboxOffset_y);
        },
  ...
  this->attackFunction = [this](bool area) { this->startAttack(area); };
  ...
  switch (type) {
    case RULE_BASED:
      this->behaviour =
          new RuleBasedBehaviour(&(this->myController), (this->observer));
      break;
    case REINFORCEMENT_1_BASED:
      this->behaviour = new ReinforcementLearningBehaviour_1(
          &(this->myController), (this->observer));
      break;
  }
...
void Level_00_GO_EnemyCharacter::startAttackCollision(bool area) {
  InstantCircleCollider hitbox{.x = this->position.x + HITBOX_OFFSET_X,
  ...
  if (this->playerNumber == PLAYER_1) {
    game->getCollisionSystem()->checkCircleHitbox(
        &hitbox, CollisionLayer::ENEMY_COLLIDER, this->basicAttackDamage);
  } else {
    game->getCollisionSystem()->checkCircleHitbox(
        &hitbox, CollisionLayer::FRIENDLY_COLLIDER, this->basicAttackDamage);
  }
}
...
void Level_00_GO_EnemyCharacter::die() { Level_00_GO_BasicCharacter::die(); }
