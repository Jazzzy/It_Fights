...
/*
 Here we load the json and png files with the data for the characters and its
 animations.
 */
Level_00_GO_BasicCharacter_AnimatedSprite::
    Level_00_GO_BasicCharacter_AnimatedSprite()
    : AnimatedSprite("prota_0.2.json", "prota_0.2.png",
                     DataMode::Aseprite_Array_Json_Data_WPivot) {
...
Level_00_GO_BasicCharacter::Level_00_GO_BasicCharacter(Scene* scene,
                                                       sf::Vector2f position,
                                                       Position playerNumber)
    : GameObject(scene),
      locationCollider(
          sf::Vector2f(LocationColliderSize_x, LocationColliderSize_y),

          // Function that returs the position in which the location collider
          // should be positioned
          [this]() -> sf::Vector2f {
            return sf::Vector2f(this->position.x - LocationColliderOffset_x,
                                this->position.y - LocationColliderOffset_y);
          },
...
void Level_00_GO_BasicCharacter::die() {
  Message winner(
      "MSG_THE_LOOSER_IS",
      MessageData{MessageData::INTEGER, .integer = this->playerNumber});
  this->scene->send(winner);
}

void Level_00_GO_BasicCharacter::manageCollisionWithVector(
    sf::Vector2f vector) {
  this->oldPosition = this->position;
...
Direction_4 Level_00_GO_BasicCharacter::calculateDirection_4(
...
void Level_00_GO_BasicCharacter::update() {
  ...
  if (attackButtonPressed) this->attackFunction(false);

  bool parryButtonPressed = this->controller->isParryButtonPressed();

  if (parryButtonPressed) this->startParry();
  ...
  this->velocity =
      (movementVector * this->walkingSpeed * Clock::Instance().getDeltaTime());
...
void Level_00_GO_BasicCharacter::startDash(sf::Vector2f direction,
                                           float magnitude, float millis,
                                           bool force) {
...
bool Level_00_GO_BasicCharacter::dash() {
  ...
  this->position.x =
      lerp(this->startingDashPosition.x, this->finalDashPosition.x, amount);
  this->position.y =
      lerp(this->startingDashPosition.y, this->finalDashPosition.y, amount);
...
bool Level_00_GO_BasicCharacter::isAttacking() { return this->attacking; }

bool Level_00_GO_BasicCharacter::isParrying() { return this->parrying; }

bool Level_00_GO_BasicCharacter::isOnCooldown() { return this->inCooldown; }
...
void Level_00_GO_BasicCharacter::startAttack(Direction_4 direction) {
  ...
  switch (direction) {
    case (Direction_4::DOWN):
      this->animatedSprite.startAnimation("ATTACK_DOWN", false,
                                          finishAnimationFunction);
      dashVector.y = 1;
      break;
    case (Direction_4::UP):
      this->animatedSprite.startAnimation("ATTACK_UP", false,
                                          finishAnimationFunction);
      dashVector.y = -1;
      break;
    case (Direction_4::LEFT):
      this->animatedSprite.startAnimation("ATTACK_LEFT", false,
                                          finishAnimationFunction);
      dashVector.x = -1;
      break;
    case (Direction_4::RIGHT):
      this->animatedSprite.startAnimation("ATTACK_RIGHT", false,
                                          finishAnimationFunction);
      dashVector.x = 1;
      break;
    default:
      this->animatedSprite.startAnimation("IDLE_DOWN", false,
                                          finishAnimationFunction);
      break;
  }
...
void Level_00_GO_BasicCharacter::startParry() {
  ...
  switch (lastDirection_4) {
    case (Direction_4::DOWN):
      this->animatedSprite.startAnimation("PARRY_DOWN", false,
                                          finishAnimationFunction);
      break;
    case (Direction_4::UP):
      this->animatedSprite.startAnimation("PARRY_UP", false,
                                          finishAnimationFunction);
      break;
    case (Direction_4::LEFT):
      this->animatedSprite.startAnimation("PARRY_LEFT", false,
                                          finishAnimationFunction);
      break;
    case (Direction_4::RIGHT):
      this->animatedSprite.startAnimation("PARRY_RIGHT", false,
                                          finishAnimationFunction);
      break;
    default:
      this->animatedSprite.startAnimation("IDLE_DOWN", false,
...
void Level_00_GO_BasicCharacter::startCooldown() {
  ...
  switch (lastDirection_4) {
    case (Direction_4::DOWN):
      this->animatedSprite.startAnimation("PARRYCOOLDOWN_DOWN", false,
                                          finishAnimationFunction);
      break;
    case (Direction_4::UP):
      this->animatedSprite.startAnimation("PARRYCOOLDOWN_UP", false,
                                          finishAnimationFunction);
      break;
    case (Direction_4::LEFT):
      this->animatedSprite.startAnimation("PARRYCOOLDOWN_LEFT", false,
                                          finishAnimationFunction);
      break;
    case (Direction_4::RIGHT):
      this->animatedSprite.startAnimation("PARRYCOOLDOWN_RIGHT", false,
                                          finishAnimationFunction);
      break;
    default:
      this->animatedSprite.startAnimation("IDLE_DOWN", false,
...
void Level_00_GO_BasicCharacter::draw(sf::RenderTarget* renderTarget) {
  ...
  characterMarker.setPosition(this->position.x + MARKER_OFFSET_X,
                              this->position.y + MARKER_OFFSET_Y);
