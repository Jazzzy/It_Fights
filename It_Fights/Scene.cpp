...
Scene::Scene(MessageBus *messageBus)
    : BusNode(Systems::S_CurrentScene, messageBus) {
  this->viewptr = NULL;
  this->localUpdateFunction = []() {};
  this->fullScreenShadersFunc = [](sf::RenderTarget *renderTarget,
...
void Scene::update() {
  ...
  for (auto iter = this->listOfGameObjects.begin();
       iter != this->listOfGameObjects.end(); ++iter) {
    (*iter)->update();
  }
}

void Scene::applyFullScreenShaders(sf::RenderTarget *renderTarget,
