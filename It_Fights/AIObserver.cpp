...
Position AIObserver::getPlayer1_2() { return (Position)this->playerPosition; }
...
MyCharacterState_Discrete AIObserver::discretizeMyCharacter(
    FightState continuousState) {
  MyCharacterState_Discrete discreteState;

  discreteState.health =
      discretizeHealth(continuousState.myState.health, HEALTH_DIVISION);
  discreteState.action = continuousState.myState.action;

  Position_Discrete discretePosition;
  {
    sf::Vector2f vectorDistance =
        continuousState.otherState.position - continuousState.myState.position;
    ...
    discretePosition.angle =
        getDirection_4FromVector(getNormalizedVector(vectorDistance));

    if (continuousState.myState.position.x <=
        WALL_X_INF) {  // Touching left wall

      if (continuousState.myState.position.y <=
          WALL_Y_INF) {  // And also the top wall

        discretePosition.wallPositions = Direction_8::LEFT_UP_8;

      } else if (continuousState.myState.position.y >=
                 WALL_Y_SUP) {  // And also the bottom wall
    ...
    } else if (continuousState.myState.position.x >=
               WALL_X_SUP) {  // Touching right wall
      ...
      if (continuousState.myState.position.y <= WALL_Y_INF) {  // Just top wall
        ...
        discretePosition.wallPositions = Direction_8::UP_8;
                 ...
                 WALL_Y_SUP) {  // Just the bottom wall
    ...
    discretePosition.lookingAtOpponent =
        (continuousState.myState.heading ==
         getDirection_4FromVector(getNormalizedVector(vectorDistance)));
  }
...
OtherCharacterState_Discrete AIObserver::discretizeOtherCharacter(
    CharacterState characterState) {
  OtherCharacterState_Discrete discreteState;

  discreteState.health =
      discretizeHealth(characterState.health, HEALTH_DIVISION);
...
void AIObserver::calculateFitness(FightState_Discrete* discreteState,
                                  FightState continuousState) {
  ...
  sf::Vector2f vectorDistance(fabs(continuousState.otherState.position.x -
                                   continuousState.myState.position.x),
                              fabs(continuousState.otherState.position.y -
                                   continuousState.myState.position.y));
...
std::ostream& operator<<(std::ostream& os, const FightState_Discrete& state) {
  return os << std::endl
            ...
            << "Distance: " << (short)state.myState.position.distance
            << std::endl
            << "Angle: " << (short)state.myState.position.angle << std::endl
            << "Walls: " << (short)state.myState.position.wallPositions
            << std::endl
            << "Looking at opponent: "
            << (short)state.myState.position.lookingAtOpponent << std::endl
...
std::string FightState_Discrete::to_short_string() {
  ...
  std::ostringstream s;
  s << this->myState.health << "_" << this->myState.position.distance << "_"
    << this->myState.position.angle << "_"
    << this->myState.position.wallPositions << "_"
    << this->myState.position.lookingAtOpponent << "_" << this->myState.action
    << "_" << this->otherState.health << "_" << this->otherState.action;

  return std::string(s.str());
}

bool operator==(const FightState_Discrete& lhs,
                const FightState_Discrete& rhs) {
