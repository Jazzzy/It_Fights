...
Window::Window(MessageBus* messageBus, Console* console,
               Collisions* collisionSystem, std::string windowName,
               int resolution_x, int resolution_y)
    : BusNode(Systems::S_Window, messageBus),
      sf_window(sf::VideoMode(resolution_x, resolution_y), windowName),
      sf_renderTexture(),
      sf_renderTexture_HighRes() {
  ...
  this->sf_renderTexture.create(this->internalResolution.first,
                                this->internalResolution.second);
  this->sf_renderTexture.setSmooth(true);

  this->sf_renderTexture_HighRes.create(this->internalResolution.first,
                                        this->internalResolution.second);
  this->sf_renderTexture_HighRes.setSmooth(true);

  this->currRealResolution = sf::Vector2u(this->internalResolution.first,
                                          this->internalResolution.second);
...
sf::Window* Window::getInternalWindowRef() { return &(this->sf_window); }
...
void Window::setCurrentInternalResolution(
    std::pair<unsigned int, unsigned int> newResolution) {
...
void Window::update() {
  ...
  /*
   Could be a good idea to have a scene with a list of drawable objects that can
   be accesed from here, maybe a scene with layers that have objects with
   shaders, this way we draw the layers in order and can move them independantly
   just calling each layer "draw" method.
   */

  // Apply here every fullscreen shader in the internal res
  game->getGameState()->getScene()->applyFullScreenShaders(
      &sf_renderTexture, &(sf_renderTexture.getTexture()));
  ...
  renderTextureSprite.scale(this->renderTextureScale.first,
                            this->renderTextureScale.second);
...
void Window::tryToResize(unsigned int x, unsigned int y) {
  bool goingBigger =
      (x > this->currRealResolution.x || y > this->currRealResolution.y);

  float desiredRatio = (float)this->internalResolution.first /
                       (float)this->internalResolution.second;
...
void Window::onNotify(Message message) {
  if (message.getEvent().compare("MSG_CLOSE_WINDOW") == 0) {
    ...
    if (message.getData().type != MessageData::PAIR_OF_INTS) {
      std::cerr << "ERROR: The data in this message should be a pair of ints"
                << std::endl;
    } else {
      this->tryToResize(message.getData().intPair.x,
                        message.getData().intPair.y);
    }
  } else if (message.getEvent().compare("MSG_RESIZE_INTERNAL_RESOLUTION") ==
             0) {
    if (message.getData().type != MessageData::PAIR_OF_INTS) {
      std::cerr << "ERROR: The data in this message should be a pair of ints"
                << std::endl;
    } else {
      this->setCurrentInternalResolution(std::make_pair(
          message.getData().intPair.x, message.getData().intPair.y));
    }
  ...
  } else if (message.getEvent().compare("MSG_TOAST") ==
             0) {  // Add a message to the console in a new line
    if (message.getData().type != MessageData::STRING_PTR) {
      std::cerr << "ERROR: The data in this message should be a string pointer"
                << std::endl;
    } else {
      this->newToastMessage(*(message.getData().string_ptr), 5.f);
      delete message.getData().string_ptr;  //@@OPTIMIZATION: We could try to
                                            //instantiate this strings on a
                                            //custom allocator/memory pool to
                                            //avoid fragmentation
    }
  }
}
...
bool Window::isOpen() { return this->sf_window.isOpen(); }
