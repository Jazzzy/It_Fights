...
AnimatedSprite::AnimatedSprite(std::string dataFilename,
  ...
  if (dataMode == DataMode::Aseprite_Array_Json_Data_WPivot) {
    ...
    int frameIndex = 0;
    for (json::iterator it = frameArray.begin(); it != frameArray.end();
         ++it, frameIndex++) {
      json frameData = *it;
    ...
    if (!this->spriteSheet.loadFromFile(resourcePath() + spriteSheetFilename)) {
      std::cerr << "ERROR: Could not load Main Character 00 spritesheet"
                << std::endl;
    }
    ...
    for (json::iterator it = animationJsonData.begin();
         it != animationJsonData.end(); ++it) {
      ...
      for (unsigned int i = currAnimData["from"];
           i <= (unsigned int)currAnimData["to"]; i++) {
...
sf::Sprite AnimatedSprite::getSprite(unsigned int index) {
  ...
  sf::Sprite sprite(this->spriteSheet,
                    sf::IntRect(fPtr->x, fPtr->y, fPtr->width, fPtr->heigth));
...
void AnimatedSprite::startAnimation(std::string name, bool loop,
  ...
  this->currentAnimationData.currentFrame =
      this->currentAnimationData.frames.begin();
  FrameData *fPtr =
      &(this->frameMap[*(this->currentAnimationData.currentFrame)]);
  this->timePerFrame = ((float)fPtr->durationMillis) / 1000.f;
}

void AnimatedSprite::goToNextFrame() {
  this->currentAnimationData.currentFrame++;
  if (this->currentAnimationData.currentFrame ==
      this->currentAnimationData.frames.end()) {
    if (loop) {
      this->currentAnimationData.currentFrame =
          this->currentAnimationData.frames.begin();
    } else {
