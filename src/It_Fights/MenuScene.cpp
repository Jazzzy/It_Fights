...
MenuScene::MenuScene(MessageBus* messageBus)
    : Scene(messageBus), titleText(this), menuOptions(this) {
...
void MenuScene::localUpdateImplemented() {
  if (!changedRes) {
    ...
    PairI intPair{.x = static_cast<int>(RESOLUTION_X),
                  .y = static_cast<int>(RESOLUTION_Y)};
