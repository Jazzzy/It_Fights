...
void Clock::setTimeScale(float timeScale) { this->timeScale = timeScale; }

float Clock::getTimeScale() { return this->timeScale; }
...
float Clock::getMeanDeltaTime() { return this->meanDeltaTime; }
...
void Clock::first20Update() {
  ...
  this->timesQueue.push_front(
      this->deltaTime);  // Meter aqui o delta time en formato sf::Time
...
void Clock::runningUpdate() {
  ...
  this->meanDeltaTime = this->meanDeltaTime - (this->timesQueue.back() / size) +
                        (this->deltaTime / size);
  this->timesQueue.pop_back();
}

unsigned long Clock::getCurrentFrameCount() { return this->currentFrameCount; }
