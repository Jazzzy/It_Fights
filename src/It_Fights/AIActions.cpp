...
void AIActions::execute(Action action) {
  switch (action) {
    ...
    case MOVE_UP: {
      this->controller->setAll(false, false,
                               getVectorFromDirection_4(Direction_4::UP));
    } break;
    case MOVE_DOWN: {
      this->controller->setAll(false, false,
                               getVectorFromDirection_4(Direction_4::DOWN));
    } break;
    case MOVE_LEFT: {
      this->controller->setAll(false, false,
                               getVectorFromDirection_4(Direction_4::LEFT));
    } break;
    case MOVE_RIGHT: {
      this->controller->setAll(false, false,
                               getVectorFromDirection_4(Direction_4::RIGHT));
    } break;
    case ATTACK:
