...
Level_00_GO_Ground::Level_00_GO_Ground(Scene *scene)
    : GameObject(scene),
      ...
      outsideCollider(
          sf::Vector2f((tileSide * tileMapWidth), (tileSide * tileMapHeight)),
          []() -> sf::Vector2f {

            sf::Vector2f origin;
            origin.x = (LVL_00_RESOLUTION_X - (tileSide * tileMapWidth)) / 2.0f;
            origin.y =
                (LVL_00_RESOLUTION_Y - (tileSide * tileMapHeight)) / 2.0f;
            return origin;
  ...
  for (float x = sidePadding_x; x < (sidePadding_x + (tileSide * tileMapWidth));
       x += tileSide) {
    for (float y = sidePadding_y;
         y < (sidePadding_y + (tileSide * tileMapHeight)); y += tileSide) {
      sf::RectangleShape rectangle;
      rectangle.setSize(
          sf::Vector2f(tileSide - outLineTHICC, tileSide - outLineTHICC));
  ...
  sf::RectangleShape rectangle;
  rectangle.setSize(sf::Vector2f((tileSide * tileMapWidth) - outLineTHICC,
                                 (tileSide * tileMapHeight) - outLineTHICC));
  ...
  noiseSprite.setTextureRect(
      sf::IntRect(0, 0, LVL_00_RESOLUTION_X, LVL_00_RESOLUTION_Y));
}
...
void Level_00_GO_Ground::drawNoise(sf::RenderTarget *renderTarget) {
  ...
  this->noiseShader.drawWithShader(&this->noiseTexture, &auxSprite,
                                   this->groundTexture.getTexture());
