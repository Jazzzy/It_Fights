...
Collisions::Collisions(MessageBus *messageBus)
    : BusNode(Systems::S_Collisions, messageBus) {
...
void Collisions::drawRectangleColliders(sf::RenderTarget *renderTarget) {
  if (currentInvertedBox != nullptr) {
    ...
    rectangle.setSize(sf::Vector2f(this->currentInvertedBox->size.x,
                                   this->currentInvertedBox->size.y));
    rectangle.setOutlineThickness(0);
    rectangle.setPosition(this->currentInvertedBox->lastOrigin.x,
                          this->currentInvertedBox->lastOrigin.y);
    rectangle.setFillColor(sf::Color(230.f, 0.f, 0.f, 40.f));
    renderTarget->draw(rectangle);
  }

  for (auto it1 = this->rectColMap.begin(); it1 != this->rectColMap.end();
       ++it1) {
    sf::RectangleShape rectangle;
  ...
  for (auto it1 = this->rectHurtMap.begin(); it1 != this->rectHurtMap.end();
       ++it1) {
    sf::RectangleShape rectangle;
    rectangle.setSize(sf::Vector2f(it1->second->size.x, it1->second->size.y));
    rectangle.setOutlineThickness(0);
  ...
  circle.setPosition(
      lastInstantCircleCollider.x - this->lastInstantCircleCollider.r,
      lastInstantCircleCollider.y - this->lastInstantCircleCollider.r);
...
unsigned int Collisions::registerRectangleCollider(
  ...
  if (rectangleCollider->colType == INVERTED_BOX) {
    ...
    this->rectHurtMap.insert(
        std::make_pair(rectangleCollider->id, rectangleCollider));
  } else {
    this->rectColMap.insert(
        std::make_pair(rectangleCollider->id, rectangleCollider));
  }
...
void Collisions::check__Box_Rect__Collisions() {
  ...
  if (this->currentInvertedBox != nullptr &&
      !this->currentInvertedBox->updated) {
    this->currentInvertedBox->lastOrigin =
        this->currentInvertedBox->funcs.getOriginFunc();
  ...
  for (auto it1 = this->rectColMap.begin(); it1 != this->rectColMap.end();
    ...
    bool inside =
        this->checkBoxRect(this->currentInvertedBox, collider, &vector);
...
bool Collisions::check2Rects(RectangleCollider *col1, RectangleCollider *col2,
  ...
  sf::Vector2f halfSize1 =
      sf::Vector2f((col1->size.x / 2.), (col1->size.y / 2.));
  sf::Vector2f halfSize2 =
      sf::Vector2f((col2->size.x / 2.), (col2->size.y / 2.));

  sf::Vector2f center1 = sf::Vector2f(col1->lastOrigin.x + halfSize1.x,
                                      col1->lastOrigin.y + halfSize1.y);
  sf::Vector2f center2 = sf::Vector2f(col2->lastOrigin.x + halfSize2.x,
                                      col2->lastOrigin.y + halfSize2.y);

  sf::Vector2f centerDiff =
      sf::Vector2f((center1.x - center2.x), (center1.y - center2.y));
  ...
  if (fabs(centerDiff.x) > fabs(centerDiff.y)) {
    // Instead of halving each component we could use normalized weights of both
    // colliders
...
bool Collisions::checkBoxRect(RectangleCollider *box, RectangleCollider *col,
                              sf::Vector2f *vector) {
  bool inside = true;
...
void Collisions::checkCircleHitbox(InstantCircleCollider *hitbox,
...
bool Collisions::checkInstantCircleRect(InstantCircleCollider *circle,
  ...
  *vector = getNormalizedVector(*vector);
  ...
  if (distance.x > (col->size.x / 2. + circle->r)) return false;
  if (distance.y > (col->size.y / 2. + circle->r)) return false;

  if (distance.x <= (col->size.x / 2.)) return true;
  if (distance.y <= (col->size.y / 2.)) return true;

  float cornerDistanceSquared = pow((distance.x - col->size.x / 2.), 2.) +
                                pow((distance.y - col->size.y / 2.), 2.);
