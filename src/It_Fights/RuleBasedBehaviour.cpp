...
RuleBasedBehaviour::RuleBasedBehaviour(EnemyCharacterController* controller,
                                       AIObserver* observer)
...
void RuleBasedBehaviour::agressiveAttack() {
  if (this->currentDiscreteState.myState.position.distance == IN_RANGE &&
      this->currentDiscreteState.myState.position.lookingAtOpponent == true) {
...
void RuleBasedBehaviour::baitAttack() {
  ...
  } else if (this->currentDiscreteState.myState.position.distance ==
             CLOSE_TO_RANGE) {
...
void RuleBasedBehaviour::deffensiveAttack() {
  if (this->currentDiscreteState.myState.position.distance != OUT_OF_RANGE &&
      this->currentDiscreteState.myState.position.lookingAtOpponent == false) {
    this->moveTowardsOpponent();

  } else if (this->currentDiscreteState.myState.position.distance !=
                 OUT_OF_RANGE &&
             this->currentDiscreteState.myState.position.lookingAtOpponent ==
                 true) {
    this->actions->execute(Action::ATTACK);
...
void RuleBasedBehaviour::moveAwayFromOpponent() {
  if (this->currentDiscreteState.myState.position.wallPositions ==
      Direction_8::NONE_8) {
    switch (this->currentDiscreteState.myState.position.angle) {
      case UP:
        this->actions->execute(Action::MOVE_DOWN);
