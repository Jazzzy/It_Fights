...
Input::Input(MessageBus *messageBus, Window *m_window)
    : BusNode(Systems::S_Input, messageBus) {
  ...
  while (this->m_window->getInternalWindowRef()->pollEvent(event)) {
    switch (event.type) {
      ...
      case sf::Event::Resized: {
        PairI intPair{.x = static_cast<int>(event.size.width),
                      .y = static_cast<int>(event.size.height)};
      ...
      case sf::Event::TextEntered: {
        if (game->isConsoleOpen()) {  // If the console is open then the key
                                      // should go to it
          if (event.text.unicode < 128 && event.text.unicode > 31 &&
              event.text.unicode != '\\') {
            Message msg_keyForConsole(
                "MSG_CONSOLE_KEYCHAR", Systems::S_Console,
                MessageData{
      ...
      case sf::Event::KeyPressed: {
        ...
        } else {  // The key is not for opening or closing the console so we
                  // should deal with it
          if (game->isConsoleOpen()) {  // If the console is open then the key
                                        // should go to it
            Message msg_keyForConsole(
                "MSG_CONSOLE_KEY", Systems::S_Console,
                MessageData{MessageData::KEYBOARD_KEY, .key = event.key.code});
            send(msg_keyForConsole);
          } else {
            // Deal with other keypresses here
            // Remember that for smooth movement I should use the sf::keyboard
            // thing
            Message msg_keypressed(
                "MSG_KEYPRESS",
                MessageData{MessageData::KEYBOARD_KEY, .key = event.key.code});
            send(msg_keypressed);
          }
      ...
      case sf::Event::JoystickButtonPressed: {
        Message msg_buttonPressed(
            "MSG_BUTTON_PRESSED",
            MessageData{MessageData::INTEGER,
                        .integer = (int)event.joystickButton.button});
...
void Input::onNotify(Message message) {
  //    std::cout << "Input System received: " << message.getEvent() <<
  //    std::endl;
}
