...
Console::Console(MessageBus *messageBus)
    : BusNode(Systems::S_Console, messageBus) {
...
void Console::update() { updateOpenness(); }

bool Console::isOpen() { return (this->current_openness != 0.0f); }

void Console::evaluateInputLine() {
  std::string strLine =
      std::string(this->inputLine.getRealString().toAnsiString());
...
void Console::onNotify(Message message) {
  if (message.getEvent().compare("MSG_TOGGLE_CONSOLE") == 0) {
  ...
  } else if (message.getEvent().compare("CONSOLE_SHOW_MSG") ==
    ...
    if (message.getData().type != MessageData::STRING_PTR) {
      ...
      this->messageLines.push_back(
          ConsoleLine(sf::String(*(message.getData().string_ptr))));
    }

    delete message.getData().string_ptr;  //@@OPTIMIZATION: We could try to
                                          //instantiate this strings on a custom
                                          //allocator/memory pool to avoid
                                          //fragmentation

  } else if (message.getEvent().compare("MSG_CONSOLE_KEYCHAR") == 0) {
    if (message.getData().type != MessageData::CHARACTER) {
      std::cerr << "ERROR: The data in this message should be a character"
                << std::endl;
    } else {
    ...
    if (message.getData().type != MessageData::KEYBOARD_KEY) {
      std::cerr << "ERROR: The data in this message should be keyboard key"
                << std::endl;
    } else {
...
void Console::draw(sf::RenderTarget *renderTarget) {
  ...
  consoleRectangle(renderTarget, 0, 0, 1, this->current_openness,
                   this->messageBackgroundColor, this->inputTextColor);

  drawMessages(renderTarget, 0, 0, 1, this->current_openness,
               this->inputTextColor, this->msgTextColor);
...
void Console::consoleRectangle(
    sf::RenderTarget *renderTarget, float origin_X, float origin_Y,
    float size_X, float size_Y, sf::Color backgroundColor,
    sf::Color outlineColor) {  // dimensions and origin in screen percentages
...
void Console::drawMessages(sf::RenderTarget *renderTarget, float origin_X,
                           float origin_Y, float size_X, float size_Y,
                           sf::Color textColor, sf::Color softTextColor) {
  sf::Vector2u renderSize = renderTarget->getSize();

  float realOrigin_X =
      (origin_X * renderSize.x) + HORIZONTAL_PADDING + OUTLINE_THICKNESS;
  float realOrigin_Y =
      (origin_Y * renderSize.y) + VERTICAL_PADDING + OUTLINE_THICKNESS;

  float realSize_Y =
      (size_Y * renderSize.y) - VERTICAL_PADDING - OUTLINE_THICKNESS * 2;

  sf::Text text(this->inputLine.getString(), this->font,
                this->pixelCharacterSize);
...

  {
    ...
    for (linesPrinted = 0; (iter != this->messageLines.rend()) &&
                           (linesPrinted <= totalLinesAvailable);
         ++iter, linesPrinted++) {  // We iterate from the end of the list;
...
void Console::drawFps(sf::RenderTarget *renderTarget, float origin_X,
                      float origin_Y, sf::Color textColor) {
  sf::Vector2u renderSize = renderTarget->getSize();
  ...
  std::string fpsString =
      "FPS: " + std::to_string(Clock::Instance().getMeanFPS());
